...
namespace BuildCompleteEcommerce.Areas.Admin.Controllers
{
    ...
    public class SlideShowController : Controller
    {
        ...
        {
            var currentSlideShows = db.SlideShows.Find(slideShow.Id);
            if (photo != null && !string.IsNullOrEmpty(photo.FileName))
            {
                ...
                var stream = new FileStream(path, FileMode.Create);
                photo.CopyToAsync(stream);
                currentSlideShows.Name = fileName;
            }
            currentSlideShows.Status = slideShow.Status;
            currentSlideShows.Title = slideShow.Title;
            currentSlideShows.Description = slideShow.Description;
            db.SaveChanges();
            return RedirectToAction("index", "slideshow", new { area = "admin" });
        }
    }
